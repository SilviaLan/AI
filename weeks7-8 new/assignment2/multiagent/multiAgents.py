...
class ReflexAgent(Agent):
    ...
    def scoress(self, fdis, nFood, ghStates, nPos):
        foodDis = fdis
        for food in nFood.asList():
            dist = manhattanDistance(food, nPos)
            if (dist < foodDis):
                foodDis = dist
        ghScore = 1000
        for state in ghStates:
            dist = manhattanDistance(nPos, state.getPosition())
            timer = state.scaredTimer
            if dist == 0 and timer > 0:
                ghScore += 9999999
            elif timer > 0 and dist < timer:
                ghScore += (1 / (2 + dist))
            elif dist < 4:
                ghScore -= dist / 100
        score = 5.0 / (1 + len(nFood.asList())) + ghScore + 1.0 / (100 + foodDis)
        return score
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        score = self.scoress(float(9999999), newFood, newGhostStates, newPos)
